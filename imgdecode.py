...
def find_datfile(dir_path):
    ...
    :return: 无
    """
    time_list = [os.stat(dir_path+'\\'+img).st_mtime for img in os.listdir(dir_path)]
    m_time = max([i for i in time_list])
    img = os.listdir(dir_path)[time_list.index(m_time)]
    decode_dat(dir_path+'\\'+img)
    return img

if __name__ == '__main__':
    # path = r'C:\Users\coder\Documents\WeChat Files\qiao18615740527\FileStorage\Image\2021-04'
    find_datfile(path)
